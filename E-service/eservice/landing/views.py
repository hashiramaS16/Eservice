...
def signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            messages.success(request, '¡Registro exitoso! Bienvenido a E-Services.')
...
def postularse_servicio(request, servicio_id):
    ...
    if perfil.rol != 'colaborador':
        messages.error(request, 'Solo colaboradores pueden postularse.')
    ...
    if Postulacion.objects.filter(servicio=servicio, colaborador=request.user).exists():
        messages.info(request, 'Ya te has postulado a este servicio.')
        return redirect('detalle_servicio', servicio_id=servicio_id)
...
def aceptar_postulacion(request, postulacion_id):
    ...
    messages.success(request, f'Has aceptado la postulación de {postulacion.colaborador.get_full_name()}.')
    return redirect('dashboard_empresa')
